...
Simple:Press
Admin Panels
$LastChangedDate: 2014-01-25 20:14:02 -0800 (Sat, 25 Jan 2014) $
$Rev: 10997 $
*/
				...
				echo '<optgroup label="'.$panel[0].'">'."\n";
					foreach ($panel[6] as $label => $data) {
						foreach ($data as $formid => $reload) {
							...
							if (isset($_GET['tab'])) {
								if ($_GET['tab'] == 'plugin') {
									if (isset($_GET['admin']) && $_GET['admin'] == $data['admin']) $sel = ' selected="selected" ';
			...
			echo '<div id="sfadminmenu">'."\n";
				foreach ($sfadminpanels as $index => $panel) {
					if (sp_current_user_can($panel[1]) || ($panel[0] == 'Admins' && ($spThisUser->admin || $spThisUser->moderator))) {
						...
						echo '<div><span class="'.$panel[4].'"></span><a href="#">'.$panel[0].'</a></div>'."\n";
						echo '</div>'."\n";
