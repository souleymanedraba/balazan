...
Simple:Press
Desc:
$LastChangedDate: 2014-01-12 09:07:31 -0800 (Sun, 12 Jan 2014) $
$Rev: 10969 $
*/
...
function sp_construct_php_error($errno, $errstr, $errfile, $errline) {
	...
	if($errno == E_NOTICE && substr($errstr, 0, 9) == 'Undefined' && $spGlobals['notices-off'] == true) return false;
	...
	if ($errno==E_NOTICE || $errno==E_RECOVERABLE_ERROR || $errno==E_WARNING || $errno==E_USER_WARNING) {
		...
		if ((defined('SP_SHOWNOTICES') && SP_SHOWNOTICES == true) || ($errno != E_NOTICE)) {
			...
			error_log('PHP '.$errortype[$errno].' - '.$errstr.' - '.$file.' - '.$line.' - '.$func, 0);
		}
	}
	return false;
}
