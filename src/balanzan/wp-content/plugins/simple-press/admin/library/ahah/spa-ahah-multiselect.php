...
function spa_populate_msbox_list($msbox, $uid, $name, $from, $to, $num) {
	...
	$out.= '<td style="vertical-align: top !important;padding:0;margin:0"">';
	$out.= '<div align="center"><strong>'.$to.' <span id="selcount">0</span></strong><br />';
	...
	$out.= '<input type="button" id="add'.$uid.'" class="button-secondary" value="'.sp_splice(spa_text('Remove From Selected List'),1).'" onclick="spjTransferSelectList(\''.$name.$uid.'\', \'temp-'.$name.$uid.'\', \''.esc_js(spa_text('List is Empty')).'\', \''.esc_js(spa_text('Maximum of 400 Users would be exceeded - please reduce the selections')).'\', \''.$name.$uid.'\')" />';
	$out.= '</div>';
	$out.= '</td>';
	$out.= '</tr>';
	$out.= '</table>';
...
//	$out.= '<input type="button"'.$disabled.' id="firstpage'.$uid.'" class="button-secondary" value="'.spa_text('First').'" onclick="spjUpdateMultiSelectList(\''.$site.'\', \''.$name.$uid.'\');" />';
...
//	$out.= '<input type="button"'.$disabled.' id="prevpage'.$uid.'" class="button-secondary" value="'.spa_text('Prev').'" onclick="spjUpdateMultiSelectList(\''.$site.'\', \''.$name.$uid.'\');" />';
	...
	$site = SFHOMEURL.'index.php?sp_ahah=multiselect&amp;sfnonce='.wp_create_nonce('forum-ahah')."&amp;page_msbox=next&amp;msbox=$msbox&amp;uid=$uid&amp;name=$name&amp;from=".urlencode($from)."&amp;num=$num&amp;offset=".($offset + $num)."&amp;max=$max&amp;filter=$filter";
//	$out.= '<input type="button"'.$disabled.' id="nextpage'.$uid.'" class="button-secondary" value="'.spa_text('Next').'" onclick="spjUpdateMultiSelectList(\''.$site.'\', \''.$name.$uid.'\');" />';
...
//	$out.= '<input type="button"'.$disabled.' id="lastpage'.$uid.'" class="button-secondary" value="'.spa_text('Last').'" onclick="spjUpdateMultiSelectList(\''.$site.'\', \''.$name.$uid.'\');" />';
